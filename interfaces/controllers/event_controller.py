...
from application.use_cases.manage_events import (
    create_event,
    get_event,
    get_all_events,
    update_event,
...
async def create_event_endpoint(event: EventCreate, db: AsyncSession = Depends(get_db)):
    ...
    return EventResponse(**created_event.model_dump())  # Usar model_dump en lugar de dict
...
async def read_event_endpoint(event_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return EventResponse(**event.model_dump())  # Usar model_dump en lugar de dict

@router.get("/events/", response_model=List[EventResponse])
async def read_all_events_endpoint(db: AsyncSession = Depends(get_db)):
    repository = EventRepository(db)
    events = await get_all_events(repository)
    return [EventResponse(**e.model_dump()) for e in events]  # Usar model_dump en lugar de dict
...
async def update_event_endpoint(event_id: int, event: EventCreate, db: AsyncSession = Depends(get_db)):
    ...
    if not updated_event:
        raise HTTPException(status_code=404, detail="Event not found")
    return EventResponse(**updated_event.model_dump())  # Usar model_dump en lugar de dict
