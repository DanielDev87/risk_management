...
class ProcessRepository:
    ...
    async def create_process(self, process: Process) -> Process:
        ...
        ).returning(ORMProcess.id, ORMProcess.macroprocess_id, ORMProcess.description)
        try:
            result = await self.session.execute(stmt)
            await self.session.commit()
            row = result.fetchone()
            if row:
                return Process(id=row.id, macroprocess_id=row.macroprocess_id, description=row.description)
        except IntegrityError as e:
            await self.session.rollback()
            raise ValueError("Process creation failed") from e

    async def get_process(self, process_id: int) -> Optional[Process]:
        stmt = select(ORMProcess).where(ORMProcess.id == process_id)
        result = await self.session.execute(stmt)
        orm_process = result.scalar_one_or_none()
        if orm_process:
            return Process(id=orm_process.id, macroprocess_id=orm_process.macroprocess_id, description=orm_process.description)
        return None
    ...
    async def get_all_processes(self) -> List[Process]:
        ...
        return [Process(id=p.id, macroprocess_id=p.macroprocess_id, description=p.description) for p in orm_processes]

    async def update_process(self, process_id: int, process: Process) -> Optional[Process]:
